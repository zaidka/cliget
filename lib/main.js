...
var DownloadHelper = Class({
  ...
  doContent: function(aMimeContentType, aRequest, aWindowContext, aForceSave) {
    if (aRequest instanceof Ci.nsIChannel)
      recentChannels.push(aRequest.QueryInterface(Ci.nsIChannel));
...
if (runtime.browserTabsRemoteAutostart) {
  events.on('http-on-opening-request', function(event) {
    let channel = event.subject;
    recentRequests.push({
      url: channel.QueryInterface(Ci.nsIChannel).originalURI.spec,
      info: getRequestInfo(channel.QueryInterface(Ci.nsIHttpChannel))
    });
    if (recentRequests.length > 50 && recentRequests.length % 50 == 0)
      recentRequests.splice(0, recentRequests.length - 50);
  }, true);
}
